...
- No new privileges
- Seccomp profile provided (`angie-seccomp.json`)
...
RUN apk add angie  # No package managers
USER root  # Never run as root
COPY --from=builder /bin/sh /bin/sh  # No shells

# ✅ DO this
FROM scratch
COPY --from=builder /usr/local/angie /usr/local/angie
USER 10001:10001
```
...
### CI Pipeline Stages
1. **Build** — Compile ANGIE with hardening flags
...
- Issues: [GitHub Issues](https://github.com/ammnt/angie/issues)
